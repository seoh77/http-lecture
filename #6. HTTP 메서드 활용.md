...
- web brower가 form의 데이터를 읽어서 HTTP 메시지 생성
...
- 예) 회원 관리 API 제공
  ...
  - 회원 수정 /members/{id} → PATCH, PUT, POST
    - PATCH : 부분 수정, PUT : 완전히 덮어버림
    - 데이터를 수정할 때는 PATCH를 사용하는 게 좋음
    - PUT은 완전히 덮어도 되는 것을 보장할 수 있을 때
    - 애매하면 POST
...
→ 대부분 POST 기반의 신규 자원 등록을 사용하고, PUT을 사용하는 비중은 거의 없다.

### HTML FORM 사용

- 웹 페이지 회원 관리
- AJAX 같은 기술을 사용해서 해결 가능
- 순수 HTML, HTML FORM은 GET, POST만을 지원하므로 제약이 있음
- HTLM FORM 사용
  - 회원 목록 /members → GET
  - 회원 등록 폼 /members/new → GET
  - 회원 등록 /members/new, /members → POST  
    → 폼과 등록 URI를 맞추는 것이 좋다
  - 회원 조회 /members/{id} → GET
  - 회원 수정 폼 /members/{id}/edit → GET
  - 회원 수정 /members/{id}/edit, /members/{id} → POST  
    → 폼과 수정 URI를 맞추는 것이 좋다
...
→ 최대한 리소스라는 개념을 가지고 URI를 설계하고, 안된다면 대체제로 컨트롤 URI를 사용
